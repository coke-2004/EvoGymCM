...
class Structure():
    ...
    def __init__(self, body, connections, label, material_scales=None, eval_index=None):
        self.body = body
        self.connections = connections

        if material_scales is None:
            self.material_scales = np.ones_like(self.body, dtype=float)
        else:
            self.material_scales = np.asarray(material_scales, dtype=float)
            if self.material_scales.shape != self.body.shape:
                raise ValueError(
                    f"material_scales shape {self.material_scales.shape} must match body shape {self.body.shape}"
                )
        self.material_scales[self.body == 0] = 1.0
...
def mutate(child, mutation_rate=0.1, num_attempts=10, material_scales=None):
    ...
    for n in range(num_attempts):
        if material_scales is not None:
            parent_material = deepcopy(np.asarray(material_scales, dtype=float))
            if parent_material.shape != child.shape:
                raise ValueError(
                    f"material_scales shape {parent_material.shape} must match body shape {child.shape}"
                )
        ...
        for i in range(child.shape[0]):
            for j in range(child.shape[1]):
                ...
                if draw(mutation) == 0: # mutation
                    ...
                    new_val = draw(pd)
                    child[i][j] = new_val
                    if material_scales is not None and new_val != old_val:
                        parent_material[i][j] = 1.0
        
        if is_connected(child) and has_actuator(child):
            connections = get_full_connectivity(child)
            if material_scales is None:
                return (child, connections)
            # print(f"mutated child:\n{child}\nparent material before mutation:\n{material_scales}\nparent material after mutation:\n{parent_material}")

            parent_material[child == 0] = 1.0
            return (child, connections, parent_material)
