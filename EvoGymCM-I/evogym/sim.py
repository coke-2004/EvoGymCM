...
class EvoSim(Sim):
    ...
    Notes:
        If a WorldObject provides `material_scales` (2D array matching its grid),
        those scales are passed to the C++ simulator during construction.
        When omitted (None), the simulator defaults to all ones (legacy behavior).
    """
    ...
    def _init_world_items(self, world: EvoWorld) -> None:
        ...
        for name in names_ordered:
            ...
            # Optional per-voxel material scaling (same shape as structure).
            # If not provided, defaults to None to preserve legacy behavior.
            material_scales = getattr(obj, 'material_scales', None)
            if material_scales is not None:
                material_scales = np.flipud(np.asarray(material_scales, dtype=float))
                if material_scales.ndim != 2 or material_scales.shape != structure.shape:
                    raise ValueError(
                        f'material_scales for {obj.name} must be 2D with shape {structure.shape}, '
                        f'got {material_scales.shape}'
                    )

            # Minimal build-time diagnostic to confirm alignment.
            # if material_scales is None:
            #     print(f'[EvoSim] build {obj.name}: structure.shape={structure.shape}, dtype={structure.dtype}, material_scales=None')
            # else:
            #     print(f'[EvoSim] build {obj.name}: structure.shape={structure.shape}, dtype={structure.dtype}, material_scales.shape={material_scales.shape}, dtype={material_scales.dtype}')
