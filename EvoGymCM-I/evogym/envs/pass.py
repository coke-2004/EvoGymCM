...
class CaveCrawlerV1(BenchmarkBase):
    ...
    def __init__(
        ...
        # make world
        self.world = EvoWorld.from_json(os.path.join(self.DATA_PATH, 'cave.json'))
        self.world.add_from_array('robot', body, 1, 2, connections=connections, material_scales=material_scales)
        ...
        self.action_space = spaces.Box(low= 0.5, high=1.8, shape=(num_actuators,), dtype=float)
...
class HoleCrawler(BenchmarkBase):
    ...
    def __init__(
        ...
        # make world
        self.world = EvoWorld.from_json(os.path.join(self.DATA_PATH, 'hole.json'))
        self.world.add_from_array('robot', body, 3, 2, connections=connections, material_scales=material_scales)
    ...
    def step(self, action):
        ...
        obs = np.concatenate((
            self.get_vel_com_obs("robot"),
            self.get_relative_pos_obs("robot"),
            self.get_floor_obs("robot", ["floor"], self.sight_dist),
            self.get_ceil_obs("robot", ["top"], self.sight_dist),
            ))
