...
class EvoWorld():
    ...
    def add_from_array(
        ...
        material_scales: Optional[np.ndarray] = None) -> None:
        """
        ...
        Args:
            ...
            material_scales (Optional[np.ndarray]): `(n, m)` array of per-voxel material scales aligned to `structure`. (default = None)
        """
        ...
        if material_scales is not None:
            ...
            if material_scales.ndim != 2 or material_scales.shape != structure.shape:
                raise ValueError(
                    f'material_scales for {name} must be 2D with shape {structure.shape}, '
                    f'got {material_scales.shape}'
                )
            # Keep the same internal orientation convention as `new_obj.grid`.
            # `WorldObject.from_array()` flips `structure` with `np.flipud`,
            # so material_scales must be flipped as well to stay aligned.
            new_obj.material_scales = np.flipud(material_scales)
        new_obj.set_pos(x, y)
...
class WorldObject():
    ...
    def __init__(self) -> None:
        ...
        # Optional per-voxel material scaling (same shape as grid).
        # When None, default behavior is equivalent to all ones.
        self.material_scales: Optional[np.ndarray] = None
    ...
    def __copy__(self,) -> WorldObject:
        ...
        if self.material_scales is None:
            out.material_scales = None
        else:
            out.material_scales = np.array(self.material_scales, copy=True)
