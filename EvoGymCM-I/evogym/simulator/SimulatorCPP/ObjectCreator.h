...
private:
	...
	vector <bool> fixed;
	vector <Edge> edges;
	Matrix <double, 1, Dynamic> pending_material_scales;
	bool material_scales_pending = false;

	double sanitize_material_scale(double raw_scale) const;
	double get_boxel_material_scale(int flat_index) const;
	void reset_edge_material_flags();
	void clear_internal_material_buffer();
...
public:
	...
	void set_pending_material_scales(const Ref<Matrix <double, 1, Dynamic>>& scales);
	void clear_pending_material_scales();
