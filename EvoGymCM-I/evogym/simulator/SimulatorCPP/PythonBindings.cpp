...
PYBIND11_MODULE(simulator_cpp, m) {
	...
	py::class_<Sim>(m, "Sim")
		...
		.def("read_object_from_array",
			[](Sim& sim,
				Matrix <double, Dynamic, Dynamic> grid,
				Matrix <double, 2, Dynamic> connections,
				string object_name,
				double x,
				double y,
				py::object material_scales) {
				// material_scales is optional; when provided, it is cast to double and must match grid shape.
				if (material_scales.is_none())
					return sim.read_object_from_array(grid, connections, object_name, x, y);
				Matrix <double, Dynamic, Dynamic> scales = material_scales.cast<Matrix <double, Dynamic, Dynamic>>();
				return sim.read_object_from_array(grid, connections, object_name, x, y, scales);
			},
			py::arg("grid"),
			py::arg("connections"),
			py::arg("object_name"),
			py::arg("x"),
			py::arg("y"),
			py::arg("material_scales") = py::none())
		.def("read_robot_from_array",
			[](Sim& sim,
				Matrix <double, Dynamic, Dynamic> grid,
				Matrix <double, 2, Dynamic> connections,
				string robot_name,
				...
				// material_scales is optional; when provided, it is cast to double and must match grid shape.
				if (material_scales.is_none())
					return sim.read_robot_from_array(grid, connections, robot_name, x, y);
				Matrix <double, Dynamic, Dynamic> scales = material_scales.cast<Matrix <double, Dynamic, Dynamic>>();
				return sim.read_robot_from_array(grid, connections, robot_name, x, y, scales);
			},
			py::arg("grid"),
			py::arg("connections"),
			py::arg("robot_name"),
