...
static double compute_brightness_factor(double s) {
	if (!std::isfinite(s))
		s = 1.0;
	// Original rule (disabled):
	// double t = s - 1.0;
	// double f = 1.0 - 0.25 * t;
	// return clamp_value(f, 0.75, 1.25);
	...
	if (s <= 0.0)
		s = 1.0;
	return 0.9 / std::sqrt(s);
}

static void apply_brightness(double& r, double& g, double& b, double s) {
	double f = compute_brightness_factor(s);
	r = clamp01(r * f);
	g = clamp01(g * f);
	b = clamp01(b * f);
}

static bool get_actuator_base_rgb(double actuation, bool is_act_vert, double& r, double& g, double& b) {
	if (actuation >= 1) {
	...
	if (actuation < 1) {
		...
		int palette_index = index + 50;
		if (is_act_vert) {
			r = blue[palette_index][0];
			g = blue[palette_index][1];
			b = blue[palette_index][2];
		} else {
			r = orange[palette_index][0];
			g = orange[palette_index][1];
			b = orange[palette_index][2];
		}
	...
	for (int i = 0; i < objects->size(); i++) {
		for (int j = 0; j < objects->at(i)->boxels.size(); j++) {
			...
			/*	setColorInterval(
				...
				has_color = get_actuator_base_rgb(percent, false, r, g, b);
			}
			...
			if (current->cell_type == CELL_ACT_V) {
				...
				has_color = get_actuator_base_rgb(percent, true, r, g, b);
			}
			...
			if (has_color) {
				apply_brightness(r, g, b, current->get_material_scale());
				glColor3f(r, g, b);
			}
			...
			// NOTE: 用户要求关闭体素边框渲染，以下边框绘制逻辑暂时注释。
			/*
			double s = current->get_material_scale();
			if (!std::isfinite(s))
				s = 1.0;
			double t = (s - 0.5) / 1.5;
			double line_width = 1.0 + t * 2.0;
			line_width = clamp_value(line_width, 1.0, 3.0);
