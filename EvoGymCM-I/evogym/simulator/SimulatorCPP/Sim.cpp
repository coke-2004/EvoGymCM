...
namespace {
	struct PendingMaterialGuard {
		ObjectCreator* creator;
		bool active;
		PendingMaterialGuard(ObjectCreator* creator_in, bool active_in) : creator(creator_in), active(active_in) {}
		~PendingMaterialGuard() {
			if (active && creator) {
				creator->clear_pending_material_scales();
			}
		}
	};

	bool flatten_grid_pair(Matrix <double, Dynamic, Dynamic>& grid,
		Matrix <double, Dynamic, Dynamic>& material_scales,
		Matrix <double, 1, Dynamic>& flat_grid,
		Matrix <double, 1, Dynamic>& flat_scales,
		int& grid_width,
		int& grid_height) {

		if (grid.rows() != material_scales.rows() || grid.cols() != material_scales.cols())
			return false;

		grid_width = grid.cols();
		grid_height = grid.rows();

		// Ensure contiguous row-major layout for both arrays (transpose if needed).
		if (grid.IsRowMajor) {
			flat_grid = Matrix <double, 1, Dynamic>(Map<Matrix <double, 1, Dynamic>>(grid.data(), grid.cols()*grid.rows()));
			flat_scales = Matrix <double, 1, Dynamic>(Map<Matrix <double, 1, Dynamic>>(material_scales.data(), material_scales.cols()*material_scales.rows()));
			return true;
		}

		grid.transposeInPlace();
		material_scales.transposeInPlace();
...
bool Sim::read_object_from_array(Matrix <double, Dynamic, Dynamic> grid, Matrix <double, 2, Dynamic> connections, string object_name, double x, double y, Matrix <double, Dynamic, Dynamic> material_scales) {
	...
	if (!flatten_grid_pair(grid, material_scales, flat_grid, flat_scales, grid_width, grid_height)) {
		cout << "Error: material_scales must match grid shape for object " << object_name << ".\n";
		return false;
	}

	creator.set_pending_material_scales(flat_scales);
	// Always clear pending material scales, even if object creation fails.
	PendingMaterialGuard guard(&creator, true);

	if (creator.read_object_from_array(object_name, flat_grid, connections, Vector2d(grid_width, grid_height), Vector2d(x, y), false)) {
		environment.save_snapshot(0);
		return true;
	}
	return false;
...
bool Sim::read_robot_from_array(Matrix <double, Dynamic, Dynamic> grid, Matrix <double, 2, Dynamic> connections, string robot_name, double x, double y, Matrix <double, Dynamic, Dynamic> material_scales) {
	...
	if (!flatten_grid_pair(grid, material_scales, flat_grid, flat_scales, grid_width, grid_height)) {
		cout << "Error: material_scales must match grid shape for robot " << robot_name << ".\n";
		return false;
	}
	...
	if (creator.read_object_from_array(robot_name, flat_grid, connections, Vector2d(grid_width, grid_height), Vector2d(x, y), true)) {
		environment.init_robot(robot_name);
		environment.save_snapshot(0);
		return true;
	}
