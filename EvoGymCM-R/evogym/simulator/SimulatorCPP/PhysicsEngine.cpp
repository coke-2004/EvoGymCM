...
void PhysicsEngine::init(
	...
	vector<SimObject*>* objects, Matrix <double, 1, Dynamic>* edge_stiffness_mod){
	...
	PhysicsEngine::edge_stiffness_mod = edge_stiffness_mod;
...
Matrix <double, 2, Dynamic> PhysicsEngine::get_pos_vel_slope(Ref<Matrix <double, 2, Dynamic>> data, int num_points) {
	...
	if (edge_stiffness_mod != NULL && edge_stiffness_mod->size() == spring_force_mag.size()) {
		spring_force_mag *= edge_stiffness_mod->array();
	}
