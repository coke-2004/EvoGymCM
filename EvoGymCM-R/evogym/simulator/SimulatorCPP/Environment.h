...
private:
	...
	Matrix <double, 1, Dynamic> edge_stiffness_mod;
	Matrix <double, 1, Dynamic> stiffness_sum;
	Matrix <int, 1, Dynamic> stiffness_count;
...
public:
	...
	void set_robot_action(string robot_name, Ref <Matrix <double, Dynamic, 2>> action, Ref <Matrix <double, Dynamic, Dynamic>> s_voxel);
	...
	// 获取边刚度调制数组（用于可视化/外部读取）
	Matrix <double, 1, Dynamic>* get_edge_stiffness_mod();
