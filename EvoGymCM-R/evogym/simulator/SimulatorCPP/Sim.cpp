...
void Sim::set_action(string robot_name, MatrixXd action) {
	...
	empty.resize(0, 0);
	environment.set_robot_action(robot_name, action, empty);
}

void Sim::set_action(string robot_name, MatrixXd action, MatrixXd s_voxel) {
	environment.set_robot_action(robot_name, action, s_voxel);
}
