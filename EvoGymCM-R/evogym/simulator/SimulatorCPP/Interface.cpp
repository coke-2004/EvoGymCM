...
// 将执行器形变映射为颜色（返回 RGB 向量，便于叠加刚度因子）
Vector3d getActuatorColor(double actuation, bool is_act_vert) {
	int index = 50;
	if (actuation > 1) {
		index = int((2.0 - actuation) * 50.0);
		index = std::max(0, std::min(index, 49));
		if (is_act_vert)
			return Vector3d(blue[index][0], blue[index][1], blue[index][2]);
		return Vector3d(orange[index][0], orange[index][1], orange[index][2]);
	}
	if (actuation < 1) {
		index = int((1.0 - actuation) * 50.0);
		index = std::max(0, std::min(index, 49));
		if (is_act_vert)
			return Vector3d(blue[index + 50][0], blue[index + 50][1], blue[index + 50][2]);
		return Vector3d(orange[index + 50][0], orange[index + 50][1], orange[index + 50][2]);
	}
	if (is_act_vert)
		return Vector3d(blue[index][0], blue[index][1], blue[index][2]);
	return Vector3d(orange[index][0], orange[index][1], orange[index][2]);
...
void Interface::render_boxels(Camera camera) {
	...
	for (int i = 0; i < objects->size(); i++) {
		for (int j = 0; j < objects->at(i)->boxels.size(); j++) {
			...
			// 直接使用 action 传入的体素刚度（按网格索引取值）
			double s = 1.0;
			if (objects->at(i)->voxel_stiffness_mod.size() > 0) {
				int grid_index = current->grid_index;
				if (grid_index >= 0 && grid_index < objects->at(i)->voxel_stiffness_mod.size())
					s = objects->at(i)->voxel_stiffness_mod(0, grid_index);
			}
			// 颜色亮度因子：Factor = 0.9 / sqrt(s)
			s = std::max(s, 1e-6);
			double factor = 0.9 / std::sqrt(s);
			...
			/*	setColorInterval(
				...
				Vector3d color = getActuatorColor(percent, false);
				r = color.x();
				g = color.y();
				b = color.z();
			}
			...
			if (current->cell_type == CELL_ACT_V) {
				...
				Vector3d color = getActuatorColor(percent, true);
				r = color.x();
				g = color.y();
				b = color.z();
			}
			...
			// 叠加刚度亮度因子
			r = std::min(1.0, r * factor);
			g = std::min(1.0, g * factor);
			b = std::min(1.0, b * factor);
			glColor3f(r, g, b);
