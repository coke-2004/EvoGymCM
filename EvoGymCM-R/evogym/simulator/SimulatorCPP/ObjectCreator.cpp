	...
	for (int y = 0; y < grid_height; y++) {
		for (int x = 0; x < grid_width; x++) {
			...
			if (current->cell_type == CELL_RIGID || current->cell_type == CELL_FIXED) {
				...
				current->edge_diag_trbl_index = make_edge(current->point_top_right_index, current->point_bot_left_index, diagonal_edge_length, rigid_structural_edge_spring_const);
				current->edge_diag_tlbr_index = make_edge(current->point_top_left_index, current->point_bot_right_index, diagonal_edge_length, rigid_structural_edge_spring_const);
				continue;
			}
			...
			if (current->cell_type == CELL_SOFT) {
				...
				current->edge_diag_trbl_index = make_edge(current->point_top_right_index, current->point_bot_left_index, diagonal_edge_length, soft_structural_edge_spring_const);
				current->edge_diag_tlbr_index = make_edge(current->point_top_left_index, current->point_bot_right_index, diagonal_edge_length, soft_structural_edge_spring_const);
				continue;
			}
			...
			if (current->cell_type == CELL_ACT_H) {
				...
				current->edge_diag_trbl_index = make_edge(current->point_top_right_index, current->point_bot_left_index, diagonal_edge_length, actuator_structural_edge_spring_const);
				current->edge_diag_tlbr_index = make_edge(current->point_top_left_index, current->point_bot_right_index, diagonal_edge_length, actuator_structural_edge_spring_const);
				continue;
			}
