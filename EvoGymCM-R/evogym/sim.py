...
class EvoSim(Sim):
    ...
    def set_action(self, robot_name: str, action: np.ndarray, s_voxel: Optional[np.ndarray] = None) -> None:
        """
        ...
        Args:
            ...
            s_voxel (Optional[np.ndarray]): `(grid_w*grid_h,)` or `(1, grid_w*grid_h)` array of voxel stiffness values.
        """
        self._check_valid_robot_name(robot_name)
        ...
        s_flat = np.asarray(s_voxel, dtype=float).reshape(-1)
        super().set_action(robot_name, informative_action, s_flat.reshape(1, -1))
