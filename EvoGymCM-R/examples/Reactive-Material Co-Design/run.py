...
def run_ga(
    ...
    ### INIT EVALUATION LOG ###
    eval_log_path = os.path.join(home_path, "eval_log.csv")
    eval_counter = [0]
    if is_continuing and os.path.exists(eval_log_path):
        with open(eval_log_path, "r", newline="") as f:
            reader = csv.reader(f)
            rows = list(reader)
            # subtract header if present
            if len(rows) > 0:
                eval_counter[0] = max(0, len(rows) - 1)

    if not os.path.exists(eval_log_path):
        with open(eval_log_path, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["eval_index", "best_reward"])
    ...
    while True:
        ...
        #better parallel
        group = mp.Group()
        eval_log_file = open(eval_log_path, "a", newline="")
        eval_writer = csv.writer(eval_log_file)

        def _make_reward_callback(target_structure: Structure):
            def _callback(reward: float):
                target_structure.set_reward(reward)
                eval_counter[0] += 1
                eval_writer.writerow([eval_counter[0], reward])
                eval_log_file.flush()
            return _callback
        for structure in structures:
            ...
            if structure.is_survivor:
                ...
                group.add_job(_run_ppo_job, ppo_args, callback=_make_reward_callback(structure))
