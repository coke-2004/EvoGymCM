...
def run_ppo(
    ...
    if args.use_stiffness_wrapper:
        vec_env = make_vec_env(env_name, n_envs=1, seed=seed, env_kwargs={
            'body': body,
            'connections': connections,
        }, wrapper_class=StiffnessActionWrapper, wrapper_kwargs={
            'body': body,
        })
    else:
        vec_env = make_vec_env(env_name, n_envs=1, seed=seed, env_kwargs={
    ...
    # if args.verbose_ppo > 0:
    #     print(f"PPO policy squash_output: {model.policy.squash_output}")
    model.learn(
