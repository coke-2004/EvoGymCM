...
def add_ppo_args(parser: argparse.ArgumentParser) -> None:
    ...
    ppo_parser.add_argument(
        # 训练设备选择（cpu/cuda/auto）
        '--device', default='auto', type=str, help='Device for PPO (e.g., cpu, cuda, auto) (default: auto)'
    )
    ppo_parser.add_argument(
    ...
    ppo_parser.add_argument(
        '--eval-interval', default=1e4, type=int, help='Number of steps before evaluating PPO model (default: 1e5)'
    )
    ppo_parser.add_argument(
        '--use-stiffness-wrapper',
        default=True,
        action=argparse.BooleanOptionalAction,
        help='Whether to include stiffness parameters in actions via wrapper (default: True)'
    )